...
public class AutorController {
    ...
    @PostMapping("/save")
    public String save(@ModelAttribute("autor") @Valid Autor autor, BindingResult result, Model model, RedirectAttributes attributes) {
        if (result.hasErrors()) {
            model.addAttribute("autor", autor);
            attributes.addFlashAttribute("error", "No se pudo guardar debido a errores.");
            return "autor/create";
        }
        autorService.crearOEditar(autor);
